...
//private var sound : CarSoundControl = null;
//sound = transform.GetComponent(CarSoundControl);
...
var		flying_timer : float = 0.0f;		// ジャンプ中タイマー.
var		is_flying : boolean = false;		// ジャンプ中？.
...
private function LateUpdate()
{
	...
	if(this.is_flying) {
		...
		var	front : Vector3;
		var	front_xz : Vector3;
	
		front = this.transform.rotation*Vector3.forward;
		front.Normalize();
	
		front_xz = front;
		front_xz.y = this.rigidbody.velocity.y/200.0f;		// 適当に上下に向くように.
		front_xz.Normalize();

		if(front_xz.magnitude > 0.01f) {

			var q : Quaternion = Quaternion.LookRotation(front_xz, Vector3.up);
...
// ジャンプ中？　チェック.
private function check_is_flying() : void
{
	this.is_flying = true;
...
function UpdateWheelGraphics(relativeVelocity : Vector3)
{
	...
	for(var w : Wheel in wheels)
	{
		...
		if(wheel.GetGroundHit(wh))
		{
			...
			// タイヤマークのオブジェクトがなくてもスキール音がなるよう、コメントアウト.
			//if(skidmarks)
			//{
				...
				*/
						...
						Skid(true, Mathf.Clamp01(skidGroundSpeed * 0.1));
					}
					else
					{
				// タイヤマークのオブジェクトがなくてもスキール音がなるよう、コメントアウト.
...
// スキール音.
// サウンド関連のスクリプトは C# に移行したけど
// スキール音のメソッドだけは Car.js から切り離せないので、
// このスクリプトに移植した.

var	play_skid : boolean = false;
var skidSound : AudioClip = null;
public var skidAudio : AudioSource = null;
...
function Skid(play : boolean, volumeFactor : float)
{
	...
	var		diff = next_volume - skidAudio.volume;

	diff = Mathf.Min(Mathf.Max(-0.1f, diff), 0.1f);

	skidAudio.volume += diff;
}
