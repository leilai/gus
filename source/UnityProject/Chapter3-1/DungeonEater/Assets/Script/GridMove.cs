...
public class GridMove : MonoBehaviour {
	...
	void Update () {
		...
		// 너무 deltaTime가 크면 벽을 통과하게 되므로
		// 큰 경우에는 조금씩 여러 번에 나누어 처리한다.
	...
	public void Move(float t)
	{
		...
		// 정면의 벽에 부딪혔는가.
		Vector3 forward_pos = pos + m_direction*0.5f; // 반Unit까지 Ray를 이동한다.
		...
		if (across || (pos-near_grid).magnitude < 0.00005f) {
			...
			// 메세지를 전송하여,OnGrid() 메소드를 부른다.
			SendMessage("OnGrid",pos);

			if (Vector3.Dot(direction_save,m_direction )< 0.00005f)
				pos = near_grid + m_direction * 0.001f;  // 조금씩 움직이지 않으면, 다시 OnGrid하므로
		}
